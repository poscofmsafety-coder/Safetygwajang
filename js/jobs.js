(function(){
  ...
  // v5: 등록일 숨김 + 마감일 표기 통일 + 이전 캐시 강제 무효화
  const CACHE_KEY='sgw_safety_jobs_v5', CACHE_MS=5*60*1000;
  ...
  function deadline(v){
    ...
    if(/채용\s*시/.test(s))return '~ 채용시';
    if(/상시/.test(s))return '~ 상시';
    const dayNames='일월화수목금토';
    const label=(y,m,d)=>{const yy=Number(y),mm=Number(m),dd=Number(d);if(!yy||!mm||!dd||mm>12||dd>31)return '';const dt=new Date(yy,mm-1,dd,12);if(dt.getFullYear()!==yy||dt.getMonth()!==mm-1||dt.getDate()!==dd)return '';return `~ ${String(mm).padStart(2,'0')}/${String(dd).padStart(2,'0')}(${dayNames[dt.getDay()]})`};
    const today=new Date();today.setHours(12,0,0,0);
    if(/오늘\s*마감/.test(s))return label(today.getFullYear(),today.getMonth()+1,today.getDate());
    if(/내일\s*마감/.test(s)){const dt=new Date(today);dt.setDate(dt.getDate()+1);return label(dt.getFullYear(),dt.getMonth()+1,dt.getDate())}
    let m=s.match(/D\s*-\s*(\d+)/i);if(m){const dt=new Date(today);dt.setDate(dt.getDate()+Number(m[1]));return label(dt.getFullYear(),dt.getMonth()+1,dt.getDate())}
    m=s.match(/(20\d{2})\s*(?:[-./]|년\s*)(\d{1,2})\s*(?:[-./]|월\s*)(\d{1,2})/);if(m)return label(m[1],m[2],m[3]);
    m=s.match(/(?:^|[^\d])(\d{2})[-./](\d{1,2})[-./](\d{1,2})(?:[^\d]|$)/);if(m)return label(2000+Number(m[1]),m[2],m[3]);
    m=s.match(/(?:^|[^\d])(\d{1,2})\s*[./-]\s*(\d{1,2})(?:\s*\([일월화수목금토]\))?/);if(m){let y=today.getFullYear();let dt=new Date(y,Number(m[1])-1,Number(m[2]),12);if(dt.getTime()<today.getTime()-45*86400000)y+=1;return label(y,m[1],m[2])}
    const stripped=s.replace(/^~\s*/,'').replace(/\s*마감(?:예정)?\s*$/,'').trim();
  ...
  function render(data,stale=false,label=''){
    ...
    grid.innerHTML=items.map(x=>{
      // 등록일은 마감일과 혼동되므로 화면에서 제외합니다. 정렬용 데이터로만 서버가 사용합니다.
      const meta=[clean(x.category),clean(x.career),clean(x.location),deadline(x.deadline)].filter(Boolean);
    ...
    if(updated){
      ...
      const prefix=data?.liveCount>0?'실시간 수집':'최근 확인 공고';
      updated.textContent=(label||`${stale?'저장된 공고':prefix} · ${when.toLocaleString('ko-KR',{month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'})}`)+` · ${items.length}건`;
    }
    return true;
  }
  ...
  async function load(force=false){
    ...
    try{
      const endpoint=force?('/api/jobs?refresh=1&client=v5&t='+Date.now()):('/api/jobs?client=v5&t='+Date.now());
      const r=await fetch(endpoint,{cache:'no-store',headers:{Accept:'application/json','Cache-Control':'no-cache'}});
      const text=await r.text();let j;try{j=JSON.parse(text)}catch(_){throw new Error('공고 응답 해석 실패')}
      ...
      if(j.refreshing&&followUpChecks<4){followUpChecks+=1;setTimeout(()=>load(false),3000)}else if(!j.refreshing){followUpChecks=0}
    }catch(e){

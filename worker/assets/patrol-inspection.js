(function(){
  ...
  let imageData='', thumbData='', resultImageData='', resultThumbData='', records=read(STORE), analyzing=false;
  ...
  function validImage(file){if(!file||!/^image\/(jpeg|png|webp)$/i.test(file.type)){alert('JPG, PNG 또는 WEBP 이미지를 선택해 주세요.');return false}if(file.size>20*1024*1024){alert('20MB 이하 이미지를 사용해 주세요.');return false}return true}
  async function setImage(file){
    if(!validImage(file))return;setState('사진을 최적화하는 중…');
  ...
  async function setResultImage(file){
    if(!validImage(file))return;
    try{resultImageData=await compress(file,1280,.76);resultThumbData=await makeThumb(resultImageData);const p=$('#patrolResultPreview'),empty=$('#patrolResultPreviewEmpty'),wrap=$('#patrolResultPreviewWrap');p.src=resultImageData;p.hidden=false;if(empty)empty.hidden=true;if(wrap)wrap.classList.add('has-image');$('#patrolResultRemoveImage').hidden=false}catch(e){alert('개선 후 사진을 읽지 못했습니다. 다른 사진을 선택해 주세요.')}
  }
  function removeResultImage(){resultImageData='';resultThumbData='';const p=$('#patrolResultPreview'),empty=$('#patrolResultPreviewEmpty'),wrap=$('#patrolResultPreviewWrap');if(p){p.hidden=true;p.removeAttribute('src')}if(empty)empty.hidden=false;if(wrap)wrap.classList.remove('has-image');if($('#patrolResultRemoveImage'))$('#patrolResultRemoveImage').hidden=true}
  function applyAI(r){
  ...
  async function requestAnalysis(payload,attempt=0){
    const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),34000);
    try{const res=await fetch('/api/ai/inspection',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload),signal:ctrl.signal});const data=await res.json().catch(()=>({}));if(res.ok&&data.ok)return data;const retryable=[429,500,502,503,504].includes(res.status);if(retryable&&attempt<1){setState('AI 응답을 다시 확인하고 있습니다…');await new Promise(r=>setTimeout(r,700));return requestAnalysis(payload,attempt+1)}throw new Error(data.error||'사진 분석을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.')}catch(e){if(e?.name==='AbortError'&&attempt<1){setState('AI 응답을 다시 확인하고 있습니다…');return requestAnalysis(payload,attempt+1)}throw e}finally{clearTimeout(timer)}
  }
  async function analyze(){
    ...
    try{const data=await requestAnalysis({image:imageData,context:$('#patrolContext').value.trim()});applyAI(data.result||{});setState('AI 초안 작성 완료 · 내용을 확인하고 수정해 주세요.','ok');$('#patrolItem').scrollIntoView({behavior:'smooth',block:'center'})}catch(e){setState(e?.name==='AbortError'?'AI 응답이 늦어 분석을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.':(e.message||'사진 분석 중 오류가 발생했습니다.'),'bad')}finally{analyzing=false;b.disabled=!imageData;b.textContent='✨ 사진 분석해서 점검일지 채우기'}
  }
  ...
  function linkRisk(row){
    ...
    if(!Array.isArray(data.hazards))data.hazards=[];if(!Array.isArray(data.shares))data.shares=[];if(data.hazards.some(x=>x.sourcePatrolId===row.id))return false;
    const current=[`순회점검 발견 당시 상태: ${row.observation||row.inspectionItem}`,row.actionStatus?`개선 확인 상태: ${row.actionStatus}`:'',row.actionDetail?`개선 결과: ${row.actionDetail}`:''].filter(Boolean).join(' / ');
    data.hazards.unshift({id:uid(),sourcePatrolId:row.id,updatedAt:new Date().toISOString(),task:row.area||'순회점검 발견사항',step:'순회점검 개선결과 확인 후 연계',type:riskHazardType(row.accidentType),source:'사업장 순회점검',scenario:row.scenario||row.inspectionItem,consequence:'',currentControl:current,basis:'',worker:row.inspector||'',workerComment:'순회점검 개선결과 확인 후 수시평가 대상으로 연계됨. '+(row.riskReason||''),controlType:'',status:row.actionStatus==='조치완료'?'검토 필요':'진행중',measure:row.improvement,owner:row.owner||'',due:row.due||'',completed:row.actionCompleted||'',evidence:'순회점검 기록 ID: '+row.id,note:'순회점검에서 자동 연계 · 기존 긴급도: '+riskPriority(row.urgency),method:data.setup.method||'three',needsReview:true});
  ...
  function collect(){
    const risk=(document.querySelector('input[name="patrolRisk"]:checked')||{}).value||'X';
    return {id:uid(),updatedAt:new Date().toISOString(),date:$('#patrolDate').value,area:$('#patrolArea').value.trim(),inspector:$('#patrolInspector').value.trim(),accidentType:$('#patrolAccidentType').value,inspectionItem:$('#patrolItem').value.trim(),observation:$('#patrolObservation').value.trim(),improvement:$('#patrolImprovement').value.trim(),scenario:$('#patrolScenario').value.trim(),urgency:$('#patrolUrgency').value,due:$('#patrolDue').value,actionDept:$('#patrolActionDept').value.trim(),owner:$('#patrolOwner').value.trim(),actionStatus:$('#patrolActionStatus').value,actionCompleted:$('#patrolActionCompleted').value,actionDetail:$('#patrolActionDetail').value.trim(),verifier:$('#patrolVerifier').value.trim(),verifiedDate:$('#patrolVerifiedDate').value,riskAssessment:risk,riskReason:$('#patrolRiskReason').value.trim(),thumbnail:thumbData||'',resultThumbnail:resultThumbData||''}
  }
  function clearForm(keepDate=true){$('#patrolForm').reset();$('#patrolDate').value=keepDate?today():'';$('#patrolUrgency').value='계획 개선';$('#patrolActionStatus').value='조치완료';radio('X');removeImage();removeResultImage();$('#patrolContext').value=''}
  function render(){
    ...
    box.innerHTML='<div class="patrol-record-grid">'+records.map(r=>`<article class="patrol-record"><div class="patrol-record-top">${r.thumbnail?`<img src="${esc(r.thumbnail)}" alt="점검 사진 썸네일">`:''}<div><span>${esc(r.date||'-')} · ${esc(r.area||'장소 미입력')}</span><h3>${esc(r.inspectionItem||'점검 기록')}</h3></div><span class="patrol-risk-pill ${r.riskAssessment==='O'?'on':''}">${r.riskAssessment==='O'?'수시평가 연계':'점검기록'}</span></div><dl><div><dt>개선요구</dt><dd>${esc(r.improvement||'-')}</dd></div><div><dt>개선결과</dt><dd>${esc(r.actionStatus||'-')} · ${esc(r.actionDetail||'-')}</dd></div><div><dt>조치담당</dt><dd>${esc([r.actionDept,r.owner].filter(Boolean).join(' · ')||'-')}</dd></div>${r.resultThumbnail?`<div class="patrol-record-result-photo"><dt>개선사진</dt><dd><img src="${esc(r.resultThumbnail)}" alt="개선 후 사진"></dd></div>`:''}</dl><div class="patrol-record-actions">${r.riskAssessment==='O'?'<a class="worker-btn" href="risk-assessment.html#hazards">위험성평가 열기 →</a>':''}<button class="mini-btn delete" data-del="${esc(r.id)}" type="button">삭제</button></div></article>`).join('')+'</div>';
  ...
  $('#patrolResultCameraBtn').addEventListener('click',()=>$('#patrolResultImageInput').click());
  $('#patrolResultImageInput').addEventListener('change',e=>setResultImage(e.target.files&&e.target.files[0]));
  $('#patrolResultRemoveImage').addEventListener('click',removeResultImage);
  $('#patrolFormClear').addEventListener('click',()=>{if(confirm('입력 내용을 초기화할까요?'))clearForm()});
  $('#patrolForm').addEventListener('submit',e=>{e.preventDefault();const row=collect();if(!row.date||!row.inspectionItem||!row.improvement||!row.accidentType||!row.actionStatus||!row.actionDetail)return alert('점검일, 재해유형, 점검사항, 개선조치 요구사항, 개선 상태, 실제 개선내용을 확인해 주세요.');if(row.riskAssessment==='O'){linkRisk(row);row.riskLinked=true}records.unshift(row);if(write(STORE,records)){render();alert(row.riskAssessment==='O'?'저장되었습니다.\n개선결과 확인 후 KRAS 수시 위험성평가에도 자동 연계했습니다. 위험성 수준을 다시 확인해 주세요.':'순회점검 개선결과까지 저장되었습니다.');clearForm();window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'})}});
  $('#patrolExport').addEventListener('click',()=>{const blob=new Blob([JSON.stringify({version:2,exportedAt:new Date().toISOString(),records},null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='순회점검일지_'+today()+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)});
  ...
  $('#patrolDate').value=today();$('#patrolVerifiedDate').value=today();render();
})();

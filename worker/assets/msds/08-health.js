...
// ⭐ 고정 예시 데이터
const HEALTH_EXAMPLE = null;

// ⭐ 기존 샘플 데이터 자동 클리어
if(!localStorage.getItem('sgw_healths_cleared_v3')){
    localStorage.removeItem('sgw_healths');
    localStorage.setItem('sgw_healths_cleared_v3', '1');
}

let healths = JSON.parse(localStorage.getItem('sgw_healths') || '[]');
function saveHealthLS(){
    try {
        localStorage.setItem('sgw_healths', JSON.stringify(healths));
    } catch(e) {
        alert('⚠ 저장 공간이 부족합니다.\n' + e.message);
    }
}

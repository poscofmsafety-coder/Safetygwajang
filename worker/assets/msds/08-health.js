...
function healthTargets(){
    ...
    (MATERIALS||[]).forEach(m=>{
        ...
        rows.forEach(c=>{
          ...
          const ci=(m.compInspections||[]).find(x=>x.cas===c.cas||x.inspection?.casNo===c.cas),ins=ci?.inspection||null;
          const local=(typeof sgwLegalForCas==='function')?sgwLegalForCas(m,c.cas):{};
          const apiTarget=ins?.ok&&ins.status==='FOUND'&&ins.legal?.specialHealthTarget===true;
          const localTarget=local.specialHealthTarget===true;
          if(apiTarget||localTarget)out.push({id:`${m.id}::${c.cas}`,productId:m.id,productName:m.name,name:c.name||ins?.matchedName||local.name||m.name,cas:c.cas,content:c.content||'-',inspection:ins,needsConfirm:false,sourceLabel:apiTarget?'KOSHA CAS 대조':'별표22·MSDS CAS 보조판정',healthFirstMonths:local.healthFirstMonths||null,healthCycleMonths:local.healthCycleMonths||null});
        });
    });
    return out;
}
...
function renderHealthTargetMaterials(){
    ...
    if(!list.length){ box.innerHTML='<div class="md:col-span-2 lg:col-span-3 text-center py-5 text-gray-400 text-xs border border-dashed rounded-lg">특수건강진단 대상으로 명시 확인된 MSDS 물질이 없습니다.<br>MSDS 등록 후 CAS 자료 검토를 실행하거나 원본 15항을 확인하세요.</div>'; return; }
    box.innerHTML=list.map(m=>`<div class="border ${m.needsConfirm?'border-amber-200 bg-amber-50/60':'border-indigo-100 bg-indigo-50/50'} rounded-lg p-3"><div class="flex items-start justify-between gap-2"><div><p class="font-bold text-gray-900">${healthEsc(m.name)}</p><p class="text-[10px] text-gray-500 mt-0.5">제품 ${healthEsc(m.productName||'-')} · 함유량 ${healthEsc(m.content||'-')}</p><p class="text-[10px] text-gray-500 font-mono mt-0.5">CAS ${healthEsc(m.cas||'-')}</p><p class="text-[10px] text-indigo-700 mt-1">${healthEsc(m.sourceLabel||'CAS 대상 확인')}${m.healthCycleMonths?` · 기본 주기 ${m.healthCycleMonths}개월`:''}</p></div><span class="${m.needsConfirm?'bg-amber-100 text-amber-800':'bg-indigo-100 text-indigo-700'} text-[9px] font-bold px-2 py-0.5 rounded">${m.needsConfirm?'별표22 확인 필요':'CAS 대상 확인'}</span></div>${m.needsConfirm?'<p class="mt-2 text-[10px] leading-4 text-amber-800">공급자 MSDS 15항의 제품 수준 정보만 확인되었습니다. CAS별 자료·별표 22 대조 후 검진대상으로 확정하세요.</p>':`<button onclick="addHealthRow('${healthEsc(m.id)}')" class="mt-2 text-[10px] font-bold text-indigo-700 underline">이 물질로 검진기록 추가</button>`}</div>`).join('');
}
...
function addHealthRow(materialId=''){
    ...
    healths.unshift({id:'H_'+Date.now()+'_'+Math.random().toString(36).slice(2,6),dept:'',name:'',materialId:materialId||'',hazardName:m?.name||'',cas:m?.cas||'',productName:m?.productName||'',assignedDate:'',examDate:'',cycleMonths:m?.healthCycleMonths||null,nextDate:'',note:m?.healthFirstMonths?`배치 후 첫 특수건강진단 ${m.healthFirstMonths}개월 이내 · 기본 주기 ${m.healthCycleMonths||''}개월`:'' ,fileReport:null});

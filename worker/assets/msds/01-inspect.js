...
const InspectCache = {
    get(cas){
        try{
            const raw = localStorage.getItem('sg_inspect_'+cas);
            if(!raw) return null;
    ...
    set(cas, data){ try{ localStorage.setItem('sg_inspect_'+cas, JSON.stringify(data)); }catch(e){} },
    del(cas){ localStorage.removeItem('sg_inspect_'+cas); },
    clearAll(){
        Object.keys(localStorage).filter(k=>k.startsWith('sg_inspect_')).forEach(k=>localStorage.removeItem(k));
    }
};

...
function renderInspectModal(cas,r){
  ...
  const publicBox=ps?`<div class="border border-emerald-200 bg-emerald-50 rounded p-3"><p class="text-xs font-bold text-emerald-900">화학물질안전원 공공데이터 참고</p><p class="text-[11px] text-emerald-900 mt-1">${escapeHtml(ps.nameKo||ps.nameEn||r.matchedName||'물질명 확인')} · ${escapeHtml(ps.casNo||cas)}</p>${ps.symptom?`<p class="text-[11px] mt-2"><b>노출 증상</b> ${escapeHtml(ps.symptom)}</p>`:''}${ps.inhale?`<p class="text-[11px] mt-1"><b>흡입</b> ${escapeHtml(ps.inhale)}</p>`:''}${ps.skin?`<p class="text-[11px] mt-1"><b>피부</b> ${escapeHtml(ps.skin)}</p>`:''}<p class="text-[10px] text-emerald-800 mt-2">공공데이터는 사고·노출 검토 보조자료입니다. 실제 사용농도·사용량·환기조건과 공급자 최신 MSDS를 최종 확인하세요.</p></div>`:'';

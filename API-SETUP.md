...
공공데이터포털에서 신청한 일반 인증키를 Cloudflare Secret에 아래 이름으로 **1개만** 저장하는 것을 권장합니다.

- `PUBLIC_DATA_API_KEY`: 공공데이터포털 일반 인증키(Decoding 권장)
...
공공데이터는 **위험성 수준, 작업중지, 법적 적합성을 자동 확정하지 않습니다.** 실제 작업조건, 측정값, 사업장 실시규정, 최신 법령 및 공급자 MSDS를 최종 확인해야 합니다.
